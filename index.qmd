...
subtitle: |
  ...
  [{{< fa display >}} Slides](https://lennartwittkuhn.com/datalad-tutorial) |
  [{{< fa brands github >}} Source](https://github.com/lnnrtwttkhn/datalad-tutorial)
  
  {{< var license-badge >}}
  {{< var doi-badge >}}
  [![Binder](https://mybinder.org/badge_logo.svg)]({{< var mybinder >}})
date: 2025-07-08
engine: knitr
execute:
  eval: false
...
{{< fa user-tie >}} I am a **Senior Specalist for Data & AI** in the public sector at PD - Berater der öffentlichen Hand and an independent **consultant and trainer** for computational reproducibility, data management, and open science

{{< fa building-columns >}} I was a **Postdoctoral Researcher** in Computational Cognitive Neuroscience at the Institute of Psychology at the University of Hamburg
...
{{< fa brain >}} I studied **the role of fast neural memory reactivation** in the human brain, applying **machine learning** & **computational modeling** to **fMRI** data
...
::: {.fragment}
- This is a hands-on tutorial with exercises.
- All exercises are documented in Jupyter Notebooks, available via [Binder](https://mybinder.org/).
- You can run all exercises in the Jupyter Notebook **in your browser without any installation**, using [Binder](https://mybinder.org/).
:::
::: {.fragment}
- Alternatively, if you have DataLad installed on your computer, you can run the exercises directly on your computer
- For details about the installation, see [this chapter in the DataLad Handbook](https://mybinder.org/)
:::

::: {.fragment}
To get started with Binder, hit this button:

[![](https://mybinder.org/badge_logo.svg){width="20%"}]({{< var mybinder >}})
:::

::: {.fragment}
- Open the link in your browser *now*, but *please wait* to explore it until we begin the first exercise.
- Be patient. Launching Binder can take a little while.
...
{{< include objectives/_objectives-introduction.qmd >}}
:::

::: {.fragment}
### Nesting of datasets with DataLad
{{< include objectives/_objectives-nesting.qmd >}}
:::

::: {.fragment}
### Provenance with DataLad
{{< include objectives/_objectives-provenance.qmd >}}
:::
...
:::
::: {.fragment}
Can you spot three hidden "jokes" in this picture?
:::
:::
::::
...
::: {.fragment .fade-in-then-semi-out}
![[Brain Imaging Data Structure (BIDS)](https://bids.neuroimaging.io/) by @gorgolewski2016 ([CC BY 4.0](https://github.com/bids-standard/bids-specification?tab=CC-BY-4.0-1-ov-file#readme))](images/bids-logo.svg){fig-align="center" width=70%}
:::
::::
:::: {.column width="50%"}

::: {.fragment .fade-in-then-semi-out}
![[Brain Imaging Data Structure (BIDS)](https://bids.neuroimaging.io/) by @gorgolewski2016 ([CC BY 4.0](https://www.nature.com/articles/sdata201644#rightslink))](images/bids-overview.png){fig-align="center" width=100%}
...
::: {style="font-size: 50%"}
Dataset from Functional Imaging Laboratory (FIL), UCL, Queen Square, Institute of Neurology, London, UK ([Source](https://www.fil.ion.ucl.ac.uk/spm/data/auditory/))
:::
...
## Exercise: Creating a DataLad dataset and saving data

[![](https://mybinder.org/badge_logo.svg)]({{< var mybinder >}})

Complete all exercises until "3. Dataset consumption".
...
- DataLad Handbook: [Create a dataset](https://handbook.datalad.org/en/latest/basics/101-101-create.html)
- DataLad Handbook: [Populate a dataset](https://handbook.datalad.org/en/latest/basics/101-102-populate.html)
- DataLad Handbook: [Modify content](https://handbook.datalad.org/en/latest/basics/101-103-modify.html)
...
Among other things, **configurations determine which files are saved in Git and which in git-annex**.
...
```bash
datalad create -c text2git test && cd test && git log
```

These two commits are made when creating a DataLad dataset with `-c text2git`:

```console
commit dfdf27528a80e052cb599ece0b4e3efef2a907eb (HEAD -> main)
Author: Lennart Wittkuhn <lennart.wittkuhn@tutanota.com>
Date:   Sun Jul 6 19:51:09 2025 +0200

    Instruct annex to add text files to Git

commit f65460b3f70454c059a24dbc97a709dd2645aa41
Author: Lennart Wittkuhn <lennart.wittkuhn@tutanota.com>
Date:   Sun Jul 6 19:51:08 2025 +0200
...
`.gitattributes` reveals the configuration of git-annex:

```{console filename=".gitattributes"}
* annex.backend=MD5E # <1>
**/.git* annex.largefiles=nothing # <2>
* annex.largefiles=((mimeencoding=binary)and(largerthan=0)) # <3>
```
1. The `MD5E` git-annex backend should be used for any file (`MD4E` is a reference to the type key).
2. git-annex will *annex*, that is *store in the object-tree*, anything that it considers to be a "large file".
By default, anything is a "large file".
Exception, anything in the `.git` folder.
3. git-annex regards all files of type "binary" as a large file.
This means, text files are not annexed but stored in Git.
<br/>
<br/>
:::
::: {.fragment}
### How can I keep a file from being annexed?
Simple version: Add your file to `.gitattributes`:
```{console filename=".gitattributes"}
[...]
...
## Exercise: Nesting modular datasets with clear provenance

[![](https://mybinder.org/badge_logo.svg)]({{< var mybinder >}})

Complete all exercises until "6. Full provenance capture and reproducibility".
...
- DataLad Handbook: [Install datasets](https://handbook.datalad.org/en/latest/basics/101-105-install.html)
- DataLad Handbook: [Dataset nesting](https://handbook.datalad.org/en/latest/basics/101-106-nesting.html)
...
## Exercise: Full provenance capture and reproducibility with DataLad
...
- DataLad Handbook: [DataLad, run! Keeping track](https://handbook.datalad.org/en/latest/basics/basics-run.html)
- DataLad Handbook: [DataLad, rerun!](https://handbook.datalad.org/en/latest/basics/101-109-rerun.html)
- DataLad Handbook: [Input and output](https://handbook.datalad.org/en/latest/basics/101-110-run2.htm)
...
:::: {.columns}
::: {.column width="60%"}
![](images/phd-git-solo.png){width="100%"}
:::
::: {.column width="40%"}
### Can you spot three hidden "jokes" in this picture?
::: {.incremental}
1. PhD Student works at odd hours, often late into the night.
1. Prof. Smith always reverts the changes made by the PhD Student.
1. PhD Student reacts within about 24 hours, while Prof. Smith takes days.
:::
:::
::::
...
::: {.fragment}
Research management with tools like Git or DataLad can feel **technical**, **complex** and **overwhelming** :fearful:
:::
...
- Streamlined syncing and updating of your work
:::
::::
:::: {.column width="50%"}
::: {.fragment}
